...
terraform {
  source = "git::git@github.com:terraform-google-modules/terraform-google-cloud-storage//modules/simple_bucket?ref=${local.versions.google_module_storage}"
}
...
inputs = {
  ...
  storage_class = coalesce(local.inputs.storage_class_override, local.region.storage_class)
  versioning    = local.inputs.versioning
}
